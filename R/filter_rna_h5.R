...
h5_list_add_mito_umis <- function(h5_list) {
  ...
  if(!"matrix_dgCMatrix" %in% names(h5_list)) {
    h5_list <- H5MANIPULATOR::h5_list_convert_to_dgCMatrix(h5_list,
                                                      target = "matrix")
  ...
  if(reconvert_matrix) {
    h5_list <- H5MANIPULATOR::h5_list_convert_from_dgCMatrix(h5_list,
                                                        target = "matrix")
  }
...
subset_h5_list_by_observations <- function(h5_list,
  ...
  sparse_matrices <- names(h5_list)[names(h5_list) %in% c("matrix","hash","ADT","peaks")]
  ...
  if(match_target == "barcodes") {
    if("matrix" %in% unconverted_matrices) {
      ...
      if(length(intersect(match_values, h5_list$matrix$barcodes)) == 0) {
        stop("No matching cells found. Check that match_target is set correctly.")
      }

      keep <- match(match_values, h5_list$matrix$barcodes)
    } else if("matrix" %in% converted_matrices) {
      if(length(intersect(match_values, colnames(h5_list$matrix_dgCMatrix))) == 0) {
        stop("No matching cells found. Check that match_target is set correctly.")
      }
...
add_h5_list_hash_results <- function(h5_list,
  ...
  list_mats <- names(h5_list)[names(h5_list) %in% c("matrix","ADT","peaks")]
  ...
  h5_list <- H5MANIPULATOR::subset_h5_list_by_observations(h5_list,
                                                      match_values = common_barcodes,
                                                      match_target = match_target)

  if(!is.null(hash_count_matrix)) {
    # filter the hash count matrix for common barcodes
    hash_count_matrix <- hash_count_matrix[, common_barcodes]

    colnames(hash_count_matrix) <- h5_list$matrix$barcodes
  ...
  sparse_matrices <- names(h5_list)[grepl("_dgCMatrix$", names(h5_list))]
  sparse_matrices <- sub("_dgCMatrix$","", sparse_matrices)
...
#'
#' @param h5_list a list object generated by running rhdf5::h5dump() on a 10x HDF5 file.
#' @param adt_metadata (optional) a data.frame or data.table containing ADT metadata. The first column should contain cell identifiers (i.e. barcodes) that match the rownames of the adt_matrix.
#' @param adt_matrix a matrix or dgCMatrix containing ADT counts. Rows should be cells, columns should be features. rownames should be cell IDs (i.e. barcodes)
#' @param match_target a character object specifying which values to use for filtering in the h5_list. If "barcodes", uses matrix/barcodes; Otherwise, looks in matrix/observations for matching values
#'
#' @return a list object with ADT data added to /ADT/ and /matrix/observations/
#' @export
#'
add_h5_list_adt_results <- function(h5_list,
                                    adt_metadata = NULL,
                                    adt_matrix,
                                    match_target = "barcodes") {
  ...
  if(match_target == "barcodes") {
    h5_barcodes <- sub("-.+","",h5_list$matrix$barcodes)
  } else {
    h5_barcodes <- h5_list$matrix$observations[[match_target]]
  }

  adt_barcodes <- rownames(adt_matrix)
  common_barcodes <- intersect(h5_barcodes, adt_barcodes)

  # Filter the adt matrix based on common barcodes
  common_adt_matrix <- common_adt_matrix[common_barcodes,]
  ...
  if(!is.null(adt_metadata)) {
    # Filter the ADT metadata
    common_adt_metadata <- adt_matrix[match(common_barcodes, adt_matrix[[1]]),]
    # Add ADT metadata to /matrix/observations
    adt_meta_cols <- names(common_adt_metadata)[-1]

    for(adt_meta_col in adt_meta_cols) {
      h5_list <- H5MANIPULATOR::set_list_path(h5_list,
                                         paste0("/matrix/observations/",adt_meta_col),
                                         common_adt_metadata[[adt_meta_col]])
    }
  }
...
split_h5_list_by_hash <- function(h5_list) {
  ...
  list_mats <- names(h5_list)[names(h5_list) %in% c("matrix","hash","ADT","peaks")]

  meta <- H5MANIPULATOR::h5_list_cell_metadata(h5_list)
  ...
  for(barcode in hto_barcodes) {
    ...
    split_h5_list[[barcode]] <- H5MANIPULATOR::subset_h5_list_by_observations(
      h5_list,
...
cat_h5_list <- function (x, y) {
  ...
  for (v in seq_along(x)) {
    ...
    if(class(x[[v]]) == "list") {
      ...
      x[[v]] <- H5MANIPULATOR::cat_h5_list(x[[v]], y[[v]])
      ...
      if(nrow(x[[v]]) == nrow(y[[v]])) {
        ...
        x_name <- names(x)[v]
        H5MANIPULATOR::stm(paste0("Can't merge ",x_name," because of incompatible rows. Skipping."))
      }
    } else {
...
reduce_h5_list <- function(h5_ll) {
  ...
  list_mats <- names(h5_ll[[1]])[names(h5_ll[[1]]) %in% c("matrix","hash","ADT","peaks")]

  for(i in seq_along(list_mats)) {
    h5_ll <- lapply(h5_ll,
                    H5MANIPULATOR::h5_list_convert_to_dgCMatrix,
                    list_mats[i])
  }

  out_list <- h5_ll[[1]]
  for(i in 2:length(h5_ll)) {
    out_list <- H5MANIPULATOR::cat_h5_list(out_list, h5_ll[[i]])
  }

  for(i in seq_along(list_mats)) {
    # Exception here for "features", which shouldn't be concatenated.
    out_list[[list_mats[i]]]$features <- h5_ll[[1]][[list_mats[i]]]$features

    out_list <- H5MANIPULATOR::h5_list_convert_from_dgCMatrix(out_list,
                                                         target = list_mats[i])
  }
