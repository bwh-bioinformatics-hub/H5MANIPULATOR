...
write_h5_list <- function(h5_list,
  ...
  if(!is.null(h5_attributes)) {
    if(h5_attributes  == "tenx") {
      h5_attributes <- H5MANIPULATOR::h5_attr_list()
    }
  }
  ...
  if(length(h5_names) > 0) {
    for(h5_name in h5_names) {
      ...
      if(class(h5_list[[h5_name]]) == "list") {
        ...
        H5MANIPULATOR::write_h5_list(h5_list[[h5_name]],
                                h5_file = h5_file,
                                h5_handle = h5_handle,
        ...
        rhdf5::h5createDataset(h5_handle,
                               ...
                               chunk = H5MANIPULATOR::choose_chunk_size(h5_list[[h5_name]]),
        ...
        bits <- H5MANIPULATOR::choose_integer_bits(h5_list[[h5_name]])
...
write_ext_h5_character <- function(object,
  ...
  h5_contents <- H5MANIPULATOR::h5ls(h5_handle)

  if(name %in% h5_contents$full_name) {
    if(h5_contents$dim == "0") {
      H5MANIPULATOR::append_ext_h5_character(object,
                                        h5_handle,
                                        name)
    } else {
    ...
    H5MANIPULATOR::create_ext_h5_character(h5_handle,
                                      name,
...
append_ext_h5_character <- function(object,
  ...
  current_size <- H5MANIPULATOR::h5dims(h5_handle,
                                   name)

  new_size <- current_size + length(object)

  H5MANIPULATOR::expand_h5_dataset(h5_handle,
                              name,
                              new_size)
...
create_ext_h5_uint <- function(h5_handle,
  ...
  if(!is.null(max_val) & is.null(bits)) {
    bits <- H5MANIPULATOR::choose_integer_bits(max_val)
...
write_10x_h5_container <- function(h5_file,
  ...
  if(data_type == "integer") {
    H5MANIPULATOR::create_ext_h5_uint(h5_handle,
                                 name = "/matrix/data",
                                 bits = data_bits)
  } else if(data_type %in% c("float", "numeric")) {
    H5MANIPULATOR::create_ext_h5_float(h5_handle,
                                  name = "/matrix/data")
  }
...
add_h5_transposed_matrix <- function(h5_file,
  ...
  mat <- H5MANIPULATOR::read_h5_dgCMatrix(h5_file,
                                     target = target,
  ...
  mat <- H5MANIPULATOR::h5_list_convert_from_dgCMatrix(mat,
                                                  target = out_target)

  H5MANIPULATOR::write_h5_list(mat,
                          h5_file = h5_file,
                          overwrite = FALSE,
                          addon = TRUE)
}
